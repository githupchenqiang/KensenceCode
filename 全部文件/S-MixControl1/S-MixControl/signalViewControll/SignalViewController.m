...
@interface SignalViewController ()<UIScrollViewDelegate,GCDAsyncUdpSocketDelegate,UITextFieldDelegate>
{
...
@property (nonatomic ,strong)NSMutableArray *InArray; //输入端口号的数组
...
@property (nonatomic ,strong)NSMutableArray *outArray; //记录更改的输出名称
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    int integer = (int)[SignalValue ShareValue].Integer/9;
    int temp = 1;
    ...
    for (int i = 0; i < [SignalValue ShareValue].GetMessage.count; i++) {
        ...
        [DataBaseHelp SelectTemp:temp Type:integer];
        
          UIButton *inButton = [self.view viewWithTag:300+i+[SignalValue ShareValue].Integer/9*145];
        NSString *Instr = [NSString stringWithFormat:@"%ld",(long)(i+300+[SignalValue ShareValue].Integer/9*145)];
    ...
    for (int i = 0 ; i < [SignalValue ShareValue].GetMessage.count; i++) {
        UIButton *button = [self.view viewWithTag:600+i+[SignalValue ShareValue].Integer/9*145+144];
      
        NSString *outstr = [NSString stringWithFormat:@"%ld",(long)(i+600+[SignalValue ShareValue].Integer/9*145+144)];
...
{   [super viewDidDisappear:animated];
    ...
    [DataBaseHelp CreatTable];
    [DataBaseHelp SelectTemp:1 Type:(int)[SignalValue ShareValue].Integer/9];
...
- (void)viewDidLoad {
    ...
    for (; i < [SignalValue ShareValue].Integer; i++) {
        NSInteger number = 300+i+[SignalValue ShareValue].Integer/9*145;
...
// 循环布置输入按钮
    for (int i = 0; i < 3; i++) {
        for (int j = 0 ;j < [SignalValue ShareValue].Integer/3; j++) {
            ...
            _InButton.tag = 300 +(3)*j+i+[SignalValue ShareValue].Integer/9*145;
            ...
            _OutButton.tag = 600+(3)*j+i+[SignalValue ShareValue].Integer/9*145+144;
...
- (void)InPressAction:(UILongPressGestureRecognizer *)gesture
{
    ...
    UIAlertAction *Action = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
         for (NSNumber *number in _InArray) {
             
            NSInteger num = number.integerValue;
            UIButton *button = [self.view viewWithTag:299+num+[SignalValue ShareValue].Integer/9*145];
    ...
    UIAlertAction *twoAc = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        NSLog(@"sun%@",_InArray);
        [_InArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NSNumber *num = obj;
            NSInteger integer = num.integerValue;
            UIButton *button = [self.view viewWithTag:299+integer+[SignalValue ShareValue].Integer/9*145];
            NSString *string = [NSString stringWithFormat:@"%ld",(long)integer+299+[SignalValue ShareValue].Integer/9*145];
            [_InArray removeAllObjects];
         
            if (_Intextfild.text.length <= 0) {
               
                NSString *title = [NSString stringWithFormat:@"%ld",(long)integer];
                [button setTitle:title forState:UIControlStateNormal];
                [_InArray removeObject:num];
                button.backgroundColor = [UIColor whiteColor];
                int temp = 1;
               int type = (int)[SignalValue ShareValue].Integer/9;
                
                [DataBaseHelp CreatTable];
                [DataBaseHelp DeleteWithTemp:temp type:type Key:string];
                [DataBaseHelp InsertIntoTemp:temp Type:type Key:string Values:title];
...
//                _SenceInName = [NSUserDefaults standardUserDefaults];
//                 [_SenceInName removeObjectForKey:string];
//                [_SenceInName setObject:title forKey:string];
...
//                _SenceInName = [NSUserDefaults standardUserDefaults];
//                [_SenceInName removeObjectForKey:string];
//                [_SenceInName setObject:str forKey:string];
              
                int temp = 1;
                int type = (int)[SignalValue ShareValue].Integer/9;
                [DataBaseHelp CreatTable];
                [DataBaseHelp DeleteWithTemp:temp type:type Key:string];
                [DataBaseHelp InsertIntoTemp:temp Type:type Key:string Values:str];
                [DataBaseHelp SelectTemp:1 Type:type];
            ...
            }else if (_Intextfild.text.length < 5 &&_Intextfild.text.length > 0)
            {
                ...
                [DataBaseHelp CreatTable];
                [DataBaseHelp DeleteWithTemp:temp type:type Key:string];
                [DataBaseHelp InsertIntoTemp:temp Type:type Key:string Values:_Intextfild.text];
                
//                _SenceInName = [NSUserDefaults standardUserDefaults];
//                [_SenceInName removeObjectForKey:string];
//                [_SenceInName setObject:_Intextfild.text forKey:string];
                
                NSLog(@"3%@ %@",string,_Intextfild.text);
            }
...
{   NSLog(@"data%@",_RemoveArray);
    ...
    UIAlertAction *Action = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        for (int i = 0; i < _outArray.count; i++) {
            
            NSNumber *num = _outArray[i];
            NSInteger tagnum = num.integerValue;
            UIButton *button = (UIButton *)[self.view viewWithTag:599+tagnum+[SignalValue ShareValue].Integer/9*145+144];
            button.backgroundColor = [UIColor whiteColor];
            button.selected = NO;
        }
    }];
    UIAlertAction *twoAc = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        [_outArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NSNumber *number = obj;
            NSInteger integer = number.integerValue;
            UIButton *button = [self.view viewWithTag:599+integer+[SignalValue ShareValue].Integer/9*145+144];
            button.backgroundColor = [UIColor whiteColor];
            button.selected = NO;
              NSString *Nstring = [NSString stringWithFormat:@"%ld",(long)integer+599+[SignalValue ShareValue].Integer/9*145+144];
            [_outArray removeObject:obj];
            if (_senceText.text.length <= 0) {
                ...
                int type = (int)[SignalValue ShareValue].Integer/9;
                [DataBaseHelp CreatTable];
                [DataBaseHelp DeleteWithTemp:temp type:type Key:Nstring];
                [DataBaseHelp InsertIntoTemp:temp Type:type Key:Nstring Values:[NSString stringWithFormat:@"%ld",(long)integer]];
                [DataBaseHelp SelectTemp:1 Type:type];
                
                
//                 _senceName = [NSUserDefaults standardUserDefaults];
//                [_senceName removeObjectForKey:Nstring];
//                [_senceName setObject:[NSString stringWithFormat:@"%ld",(long)integer] forKey:Nstring];
//                NSLog(@"%4ld %@",(long)integer , Nstring);
                [_outArray removeAllObjects];
                ...
                [DataBaseHelp CreatTable];
                [DataBaseHelp DeleteWithTemp:temp type:type Key:Nstring];
                [DataBaseHelp InsertIntoTemp:temp Type:type Key:Nstring Values:str];
                
//                 _senceName = [NSUserDefaults standardUserDefaults];
//                [_senceName removeObjectForKey:Nstring];
//                [_senceName setObject:str forKey:Nstring];
                NSLog(@"5%@ %@",str , Nstring);
                [_outArray removeAllObjects];
            ...
            }else
            {
                ...
                [DataBaseHelp CreatTable];
                [DataBaseHelp DeleteWithTemp:temp type:type Key:Nstring];
                [DataBaseHelp InsertIntoTemp:temp Type:type Key:Nstring Values:_senceText.text];
                [DataBaseHelp SelectTemp:temp Type:type];
                
//                 _senceName = [NSUserDefaults standardUserDefaults];
//                [_senceName removeObjectForKey:_senceText.text];
//                [_senceName setObject:_senceText.text forKey:Nstring];
//                NSLog(@"6%@ %@",_senceText , Nstring);
                [_outArray removeAllObjects];
            }
            [_outArray removeAllObjects];
...
{   [_InArray removeAllObjects];
    ...
    NSLog(@"InArray%@",_InArray);
    
    for (int i = 0; i < [SignalValue ShareValue].GetMessage.count; i ++) {
        
        NSInteger select = (NSInteger)(btn.tag - (299+[SignalValue ShareValue].Integer/9*145));
       _temp = [NSNumber numberWithInteger:select];
        ...
        if ([SignalValue ShareValue].GetMessage[i] == _temp) {
            ...
            UIButton *button = [self.view viewWithTag:count + 599+[SignalValue ShareValue].Integer/9*145+144];
    ...
    for (int i=0; i<_TagArray.count; i++) {
        ...
        if (btn.tag == button.tag) {
            button.backgroundColor = [UIColor orangeColor];
            NSInteger vale = (NSInteger)(btn.tag - (299+[SignalValue ShareValue].Integer/9*145));
            NSNumber *number = [NSNumber numberWithInteger:vale];
            [_InArray addObject:number];
...
{
     NSLog(@"Out%@",_outArray);
     NSLog(@"UnArray%@",_UnArray);
     NSLog(@"2remove%@",_RemoveArray);
  
    [_outArray removeAllObjects];
    if (sender.selected == NO) {
        sender.selected = YES;
       self.sendInt = (NSInteger)(sender.tag - (599+[SignalValue ShareValue].Integer/9*145+144)) ;
...
-(void)Action4Clear:(UIButton *)Clear
{
    ...
    for (int i = 0; i < _RemoveArray.count; i++) {
        ...
        NSInteger integer = num.integerValue;
        UIButton *outButton = (UIButton *)[self.view viewWithTag:integer + 599+[SignalValue ShareValue].Integer/9*145+144];
...
{   [_UnArray removeAllObjects];
    ...
    for (int i = 0; i < [SignalValue ShareValue].Integer; i++) {
        _OutButton = [self. view viewWithTag:600+i+[SignalValue ShareValue].Integer/9*145+144];
        if (_OutButton.selected== NO) {
            self.isAllSeleted = NO;
            NSInteger witch = (NSInteger)(_OutButton.tag - 599+[SignalValue ShareValue].Integer/9*145+144);
            NSNumber *num = [NSNumber numberWithInteger:witch];
            [_SendArray addObject:num];
            NSLog(@"sendArray%@",_SendArray);
            break;
    ...
    if (self.isAllSeleted == NO ) {
        for (int i = 0; i < [SignalValue ShareValue].Integer; i++) {
           _OutButton = [self. view viewWithTag:600+i+[SignalValue ShareValue].Integer/9*145+144];
            ...
            NSInteger outbut = (NSInteger)(_OutButton.tag - (599+[SignalValue ShareValue].Integer/9*145+144));
            NSNumber *number = [NSNumber numberWithInteger:outbut];
            [_SendArray addObject:number];
            NSLog(@"send%@",_SendArray);
        ...
        for (int i = 0; i < [SignalValue ShareValue].Integer ; i++) {
            ...
            NSInteger intOut = (NSInteger)(_OutButton.tag - (599+[SignalValue ShareValue].Integer/9*145+144));
