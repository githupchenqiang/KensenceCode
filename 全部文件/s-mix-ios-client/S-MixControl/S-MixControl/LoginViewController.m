...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.SButton.selected = [_default boolForKey:@"Select"];
...
//    _userDefault = [NSUserDefaults standardUserDefaults];
//    NSString *Host = [_userDefault objectForKey:@"IPAdress"];
//    NSString *port = [_userDefault objectForKey:@"PortValues"];
//    unsigned short portval = [port intValue];
//    
//    if (_text4ip.text != 0 && _text4port.text != 0) {
//        
//    }
//}
...
- (void)viewDidLoad {
    ...
    viwe.frame = CGRectMake(KscreenWidth/3, CGRectGetMaxY(_text4port.frame)+20, KscreenWidth/3, 52);
    viwe.layer.borderWidth = 0.6;
    viwe.layer.borderColor = [UIColor blackColor].CGColor;
    viwe.layer.cornerRadius = 7 ;
    viwe.layer.masksToBounds = YES;
    [self.view addSubview:viwe];
    
    //Pro的选择类型按钮
    _SButton = [UIButton buttonWithType:UIButtonTypeSystem];
    _SButton.frame = CGRectMake(KscreenWidth/3, CGRectGetMaxY(_text4port.frame)+20, KscreenWidth/6, 52);
...
//    _SButton.layer.borderColor = [UIColor blackColor].CGColor;
    ...
    _Pbutton.frame = CGRectMake(CGRectGetMaxX(_SButton.frame), CGRectGetMaxY(_text4port.frame)+20, KscreenWidth/6, 52);
...
//    _Pbutton.layer.borderColor = [UIColor blackColor].CGColor;
    ...
    _button.frame = CGRectMake(KscreenWidth/3,CGRectGetMaxY(_SButton.frame) + 20, KscreenWidth/3, 52);
...
- (void)ProButtonAction:(UIButton *)sender
{
    ...
    if (!sender.tag) {
        ...
        [_SButton setImage:image forState:UIControlStateNormal];
       _default = [NSUserDefaults standardUserDefaults];
        [_default setBool:sender.selected forKey:@"Select"];
...
- (void)udpSocket:(GCDAsyncUdpSocket *)sock didReceiveData:(NSData *)data fromAddress:(NSData *)address withFilterContext:(id)filterContext
{
    ...
    }else if (cmd == 0x27)
    {
        ...
        for(int i = 0; i < _count; i++)
        {
            ...
            [[SignalValue ShareValue].GetMessage addObject:number];
            NSLog(@"34%@",[SignalValue ShareValue].GetMessage);
        }

    }

    _dispath++;
    if (_dispath == 2 && [SignalValue ShareValue].GetMessage!=nil) {
        //视图跳转
