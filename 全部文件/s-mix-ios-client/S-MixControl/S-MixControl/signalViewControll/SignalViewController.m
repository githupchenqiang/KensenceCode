...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    for (int i = 0; i < [SignalValue ShareValue].GetMessage.count; i++) {
          UIButton *inButton = [self.view viewWithTag:300+i];
        NSString *Instr = [NSString stringWithFormat:@"%d",i+300];
    ...
    for (int i = 0 ; i < [SignalValue ShareValue].GetMessage.count; i++) {
        UIButton *button = [self.view viewWithTag:600+i];
      
        NSString *outstr = [NSString stringWithFormat:@"%d",i+600];
...
- (void)viewDidLoad {
    ...
    if (result == SQLITE_OK) {
        ...
        const char *sql = "CREATE TABLE IF NOT EXISTS t_name (id integer PRIMARY KEY AUTOINCREMENT,name text NOT NULL , type integer NOT NULL);";
        char *erroMsg = NULL;
...
- (void)InPressAction:(UILongPressGestureRecognizer *)gesture
{
    ...
    UIAlertAction *Action = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
         for (NSNumber *number in _InArray) {
            ...
            UIButton *button = [self.view viewWithTag:299+num];
    ...
    UIAlertAction *twoAc = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        [_InArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            UIButton *button = [self.view viewWithTag:299+integer];
            NSString *string = [NSString stringWithFormat:@"%ld",(long)integer+299];
...
{   NSLog(@"data%@",_RemoveArray);
    ...
    UIAlertAction *Action = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        for (int i = 0; i < _outArray.count; i++) {
            ...
            UIButton *button = (UIButton *)[self.view viewWithTag:599+tagnum];
            button.backgroundColor = [UIColor whiteColor];
            button.selected = NO;
        }
    }];
...
{   [_InArray removeAllObjects];
    ...
    for (int i = 0; i < [SignalValue ShareValue].GetMessage.count; i ++) {
        ...
        NSInteger select = (NSInteger)(btn.tag - 299);
       _temp = [NSNumber numberWithInteger:select];
        ...
        if ([SignalValue ShareValue].GetMessage[i] == _temp) {
            ...
            UIButton *button = [self.view viewWithTag:count + 599];
    ...
    for (int i=0; i<_TagArray.count; i++) {
        ...
        if (btn.tag == button.tag) {
            button.backgroundColor = [UIColor orangeColor];
            NSInteger vale = (NSInteger)(btn.tag - 299);
...
- (void)OutbuttonAction:(UIButton *)sender
{
    ...
    if (sender.selected == NO) {
        sender.selected = YES;
       self.sendInt = (NSInteger)(sender.tag - 599) ;
    ...
    sender.backgroundColor = [UIColor whiteColor];
        sender.selected = NO;
        NSInteger intnum = (NSInteger)(sender.tag - 599);
...
-(void)Action4Clear:(UIButton *)Clear
{
    ...
    for (int i = 0; i < _RemoveArray.count; i++) {
        ...
        NSInteger integer = num.integerValue;
        UIButton *outButton = (UIButton *)[self.view viewWithTag:integer + 599];
...
- (void)SendMessage:(UIButton *)Send
{
    ...
    unsigned char dobuf[256] = {0};
    for (int i = 0; i < sizeof(sendout)+1; i++) {
        if (i == 0) {
            dobuf[i] = (unsigned char)Intag;
    ...
    if ([SignalValue ShareValue].ProCount == 1) {
        ...
        NSInteger length = make_pack_5555(METHOD_SET, CMD_SCREEN_SWITCH_STATUSMAP, i,dobuf, buf1);
...
{   [_UnArray removeAllObjects];
    ...
    for (int i = 0; i < [SignalValue ShareValue].Integer; i++) {
        _OutButton = [self. view viewWithTag:600+i];
        if (_OutButton.selected== NO) {
            self.isAllSeleted = NO;
            NSInteger witch = (NSInteger)(_OutButton.tag - 599);
    ...
    if (self.isAllSeleted == NO ) {
        for (int i = 0; i < [SignalValue ShareValue].Integer; i++) {
           _OutButton = [self.view viewWithTag:600+i];
            ...
            NSInteger outbut = (NSInteger)(_OutButton.tag - 599);
        ...
        for (int i = 0; i < [SignalValue ShareValue].Integer ; i++) {
            ...
            NSInteger intOut = (NSInteger)(_OutButton.tag - 599);
...
{
    NSLog(@"Now %@",[SignalValue ShareValue].GetMessage);
    
    [[SignalValue ShareValue].GetMessage removeAllObjects];
    NSLog(@"remove %@",[SignalValue ShareValue].GetMessage);
