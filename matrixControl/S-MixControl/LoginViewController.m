...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (inte == 2) {
        ...
        [button setTitle:NSLocalizedString(@"matrix-pro",@"") forState:UIControlStateNormal];
        [SignalValue ShareValue].ProCount = 2;
        
    }else if (inte == 1){
        UIButton *button = (UIButton *)[self.view viewWithTag:2998];
          [button setTitle:@"matrix-e" forState:UIControlStateNormal];
...
//    _BackView.backgroundColor = WhiteColor;
    ...
    UIImage *LogImage = [UIImage imageNamed:@"matrixControlLogin"];
    ...
    ProductLabel.text = NSLocalizedString(@"matrix control",@"");
...
//    _text4ip.layer.masksToBounds = YES;
    ...
    if (inte == 1) {
        [_SButton setTitle:@"matrix-e" forState:UIControlStateNormal];
    }else if (inte == 2)
    {
       [_SButton setTitle:@"matrix-pro" forState:UIControlStateNormal];
    }else
    {
...
- (void)ProButtonAction:(UIButton *)sender
{
    ...
    [PRoButton setTitle:@"matrix-e" forState:UIControlStateNormal];
...
//    PRoButton.layer.masksToBounds = YES;
    ...
    [SMButton setTitle:@"matrix-pro" forState:UIControlStateNormal];
...
- (void)SMButton:(UIButton *)sender
{
    _isChoose = NO;
    UIButton *button = (UIButton *)[self.view viewWithTag:2999];
    [button setTitle:@"matrix-pro" forState:UIControlStateNormal];
    [SignalValue ShareValue].ProCount = 2;
...
//        }
    ...
    _dispath++;
    if ([SignalValue ShareValue].GetMessage.count > 0) {
        dispatch_async(dispatch_get_main_queue(), ^{
