...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    for (int i = 0; i < [SignalValue ShareValue].Integer ; i++) {
        ...
        NSString *value = [[NSUserDefaults standardUserDefaults]objectForKey:[Key stringByAppendingString:[SignalValue ShareValue].deviceIP]];
    ...
    if ([SignalValue ShareValue].ProCount == 2 || [SignalValue ShareValue].ProCount == 1) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            NSString *value = [[NSUserDefaults standardUserDefaults]objectForKey:[string stringByAppendingString:[SignalValue ShareValue].deviceIP]];
...
- (void)SetUpLabel
{
    for (int i = 0; i < [SignalValue ShareValue].Integer; i++) {
        for (int j = 0; j < 1; j++) {
            UILabel *Inlabel = [[UILabel alloc]initWithFrame:CGRectMake(40 + ScrollerWith/100*j,KScreenHeight/16 + KScreenHeight/10.6*i,ScrollerWith/7,40)];
            ...
            _OutScroller.backgroundColor = [UIColor colorWithRed:14/255.0 green:23/255.0 blue:30/255.0 alpha:1];
...
//            _Button.layer.borderWidth = 0.5f;
            ...
            NSString *sence = NSLocalizedString(@"Scene", @"");
...
//        button.backgroundColor = [UIColor whiteColor];
    ...
    UIAlertAction *twoAc = [UIAlertAction actionWithTitle:NSLocalizedString(@"Sure",@"") style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        ...
        if (_Intextfild.text.length <= 0) {
            ...
            NSString *title = [NSString stringWithFormat:@"Scene%ld",(long)_buttonValue];
...
//            button.backgroundColor = [UIColor whiteColor];
            ...
            [DataBaseHelp CreatTable];
            [DataBaseHelp DeleteWithIP:[SignalValue ShareValue].deviceIP Temp:integer type:tage Key:ConseverString];
            [DataBaseHelp InsertIntoIP:[SignalValue ShareValue].deviceIP Temp:integer Type:tage Key:ConseverString Values:title];
            ...
            [DataBaseHelp CreatTable];
            [DataBaseHelp DeleteWithIP:[SignalValue ShareValue].deviceIP Temp:integer type:tage Key:ConseverString];
            [DataBaseHelp InsertIntoIP:[SignalValue ShareValue].deviceIP Temp:integer Type:tage Key:ConseverString Values:str];
        ...
        }else if (_Intextfild.text.length < 5 &&_Intextfild.text.length > 0)
        {
            ...
            [DataBaseHelp CreatTable];
            [DataBaseHelp DeleteWithIP:[SignalValue ShareValue].deviceIP Temp:integer type:tage Key:ConseverString];
            [DataBaseHelp InsertIntoIP:[SignalValue ShareValue].deviceIP Temp:integer Type:tage Key:ConseverString Values:_Intextfild.text];
...
- (void)SetUpLoadButton
{
    ...
    _LoadButton.frame = CGRectMake(KScreenWith/1.45 + KScreenWith/10, KScreenWith/4.3+(KScreenHeight/9)*5, KScreenWith/7, KScreenHeight/12);
//    _LoadButton.backgroundColor = [UIColor whiteColor];
    ...
    [_LoadButton setBackgroundImage:imageBu forState:UIControlStateNormal];
    [_LoadButton setTitle:NSLocalizedString(@"Load",@"") forState:UIControlStateNormal];
...
- (void)udpSocket:(GCDAsyncUdpSocket *)sock didSendDataWithTag:(long)tag
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self.hud.labelText = NSLocalizedString(@"load success",@"");
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
           ...
           // _Astring  = [SignalValue ShareValue].GetMessage[0];
            for (int i = 0; i <[SignalValue ShareValue].GetMessage.count; i++) {
                ...
                NSString *value = [[NSUserDefaults standardUserDefaults]objectForKey:[strVale stringByAppendingString:[SignalValue ShareValue].deviceIP]];
                if (value == nil) {
                ...
                }else
                {
                    ...
                    NSString *stringValue = [[NSUserDefaults standardUserDefaults]objectForKey:[strVale stringByAppendingString:[SignalValue ShareValue].deviceIP]];
